...
import { fetchOrders, refundOrder, canSeeFinancials } from '../../lib/api';
...
function StatusBadge({ status }) {
  const styles = {
    ...
    comp: 'bg-purple-500/20 text-purple-400 border-purple-500/30',
  };
...
export default function OrdersList() {
  ...
  const loadOrders = useCallback((currentPage, currentSearch) => {
    setLoading(true);
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        {pagination.total > 0 && (
          <span className="text-sm text-gray-500">{pagination.total} total orders</span>
        )}
      </div>
            ...
            <div className="flex items-center justify-between pt-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={page <= 1}
                  className="flex items-center gap-1 px-3 py-1.5 bg-pavilion-700 border border-pavilion-600/50 rounded-lg text-sm text-gray-400 hover:text-white transition-all disabled:opacity-30 disabled:cursor-not-allowed"
                >
